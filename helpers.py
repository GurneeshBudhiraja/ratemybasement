...
def main():
  try:
    result_data = check_data(form_data={'address': '34 Keon Pl, Scarborough, Toronto, On M1B 3C9 Can', 'rent': None, 'years': None, 'clean': 'Dirty', 'maintain': 'Needs Improvement', 'amenities': 'Lacking', 'neighbourhood': 'Unfavorable', 'review': ''})
    print(result_data)
    if result_data:
      final_data = prepare_data(form_data={'address': '34 Keon Pl, Scarborough, Toronto, On M1B 3C9 Can', 'rent': None, 'years': None, 'clean': 'Dirty', 'maintain': 'Needs Improvement', 'amenities': 'Lacking', 'neighbourhood': 'Unfavorable', 'review': ''})
    print(final_data)
  ...
  try:
    for key,value in (form_data).items():
      if key=="address":
        ...
        if (form_data[key]).strip().title() not in ("Dirty","Average","Clean"):
          return False
      elif key=="maintain":
        if (form_data[key]).strip().title() not in ("Needs Improvement","Adequate","Well Maintained"):
          return False
      elif key=="amenities":
        if (form_data[key]).strip().title() not in ("Lacking","Basic","Good"):
          return False
      elif key=="neighbourhood":
        if (form_data[key]).strip().title() not in ("Unfavorable","Okay","Safe"):
          return False
      elif key=="review":
